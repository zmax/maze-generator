...
import type { Cell, GenerationStepCallback, MazeGenerationAlgorithm, MazeGeneratorOptions, SolverStepCallback } from './types';
 ...
 * @param height 迷宮高度
 */
async function createAndSolveMaze(width: number, height: number, algorithm: MazeGenerationAlgorithm, options?: MazeGeneratorOptions): Promise<void> {
 ...
 * @param seed 隨機種子
 */
async function createAndVisualizeMaze(width: number, height: number, algorithm: MazeGenerationAlgorithm, seed?: number) {
  console.log(`\n--- 視覺化 ${algorithm} 演算法生成過程 (種子: ${seed}) ---`);
  
  // 一個簡單的、用於在主控台清除並重繪的視覺化回呼函式
  const onStepCallback: GenerationStepCallback = async (step) => {
    console.clear();
    console.log(`視覺化 ${algorithm} 生成中...`);
    
    // 建立一個臨時路徑來突顯演算法正在處理的儲存格
    const highlightPath: Cell[] = [];
    if (step.walkPath) highlightPath.push(...step.walkPath); // Wilson's
    if (step.stack) highlightPath.push(...step.stack); // Recursive Backtracker
    if (step.activeSet) highlightPath.push(...step.activeSet); // Prim's, Growing Tree
    if (step.currentCell) highlightPath.push(step.currentCell); // 突顯當前儲存格
    
    drawMazeToConsole(step.grid, highlightPath);
 ...
 * @param seed 隨機種子
 */
async function visualizeSolving(width: number, height: number, algorithm: MazeGenerationAlgorithm, seed?: number) {
  ...
  // 2. 建立求解器的視覺化回呼函式
  const onSolveStep: SolverStepCallback = async (step) => {
    console.clear();
    console.log(`視覺化求解中...`);
    drawMazeToConsole(step.grid, {
  ...
  // 3. 建立求解器並執行求解
  const solver = new MazeSolver(grid, { onStep: onSolveStep });
  const finalPath = await solver.solve(start, end);

  // 4. 顯示最終結果
  console.clear();
  console.log(`求解完畢！`);
  drawMazeToConsole(grid, finalPath);
}
